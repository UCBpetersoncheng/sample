...
class LambdaFunction(SchemeValue):
    ...
    def __init__(self, formals, body, env):
        ...
        self.body = Pair(Symbol.string_to_symbol("begin"), body)
        self.env = env
    ...
    def apply_step(self, args, evaluation):
        "*** YOUR CODE HERE ***"
        Evaluation.check_form(evaluation, len(args), len(args), self.formals)
        _eval_ = evaluation.env.make_call_frame(self.formals, args)
        evaluation.set_expr(self.body, _eval_)
...
class EnvironFrame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        def helper(_formals, _vals):
            if _formals != NULL and _vals != []:
                New_Frame.define(_formals.car, _vals[0])
                if scm_pairp(_formals.cdr):
                    helper(_formals.cdr, _vals[1:])
                elif scm_symbolp(_formals.cdr):
                    New_Frame.define(_formals.cdr, scm_list(*_vals))
                    
            elif _vals != []:
                raise SchemeError("mismatched args in formals and vals")
...
class Evaluation:
    ...
    def do_if_form(self):
        ...
        "*** YOUR CODE HERE ***"
        if cond == TRUE:
            self.set_expr(self.expr.nth(2))
        else:
            self.set_expr(self.expr.nth(3))
    ...
    def do_and_form(self):
        ...
        "*** YOUR CODE HERE ***"
        if not self.full_eval(self.expr.nth(1)):
            self.set_value(FALSE)
        elif not self.full_eval(self.expr.nth(2)):
            self.set_value(FALSE)
        else:
            self.set_expr(self.expr.nth(2))
    ...
    def do_or_form(self):
        ...
        "*** YOUR CODE HERE ***"
        if self.full_eval(self.expr.nth(1)):
            self.set_expr(self.expr.nth(1))
        elif self.full_eval(self.expr.nth(2)):
            self.set_expr(self.expr.nth(2))
        else:
            self.set_expr(FALSE)
    ...
    def do_cond_form(self):
        ...
        for i in range(1, num_clauses):
            ...
            if clause.car is self._ELSE_SYM and i == num_clauses-1:
                ...
                """if test == TRUE and not clause.cdr.nullp():
                    self.set_expr(clause.nth(1))
                    return"""
            if test:
                if clause.length() == 1:
                    self.set_value(test)
                    ...
                    "*** YOUR CODE HERE ***"
                    func = clause.nth(2)
                    self.set_expr(scm_list(func,test))
    ...
    def do_set_bang_form(self):
        ...
        "*** YOUR CODE HERE ***"
        sym = self.expr.nth(1)
        expr = self.full_eval(self.expr.nth(2))
        self.env[sym] = expr
        self.set_value(UNSPEC)
    ...
    def do_define_form(self):
        ...
        if target.symbolp():
            ...
            "*** YOUR CODE HERE ***"
            self.env.define(target.car, LambdaFunction(target.cdr, self.expr.cdr.cdr, self.env))
            self.set_value(UNSPEC)
    ...
    def do_let_form(self):
        ...
        vals = []
        "*** YOUR CODE HERE ***"
        for k in range(0, bindings.length()):
            value = bindings.nth(k).cdr.car
            vals += [self.full_eval(value)]
        for j in range(bindings.length()-1, -1, -1):
            sym = bindings.nth(j).car
            symbols = scm_cons(sym, symbols)
    ...
    def do_call_form(self):
        ...
        def scm_list_to_list(x):
            result = []
            while True:
                if scm_symbolp(x):
                    result += [self.full_eval(x)]
                    break
                elif scm_pairp(x):
                    result += [self.full_eval(x.car)]
                    x = x.cdr
                else:
                    break
            return result
        
        op.apply_step(scm_list_to_list(args), self)
    ...
    def check_formals(formal_list):
        ...
        x = formal_list
        while True:
            if not scm_symbolp(x.car):
                return False
            elif scm_pairp(x.cdr):
                x = x.cdr
            elif scm_symbolp(x.cdr):
                return True
            elif scm_nullp(x.cdr):
                return True
            else:
                return False
...
def scm_read():
    def read_tail():
        ...
        (2 3) when positioned at the at in "(1 ^ 2 3)", returns
        ...
        "*** YOUR CODE HERE ***"
        if syntax == ')':
            input_port.pop()            # Removes the ) from buffer
            return NULL
        elif syntax == '.':
            input_port.pop()            # Removes the . from buffer
            result = scm_read()         # Evaluates the last expression
            input_port.pop()            # Removes the final ) from buffer
            return result
        first = scm_read()
        rest = read_tail()
    ...
    if syntax == NUMERAL:
        ...
        "*** YOUR CODE HERE ***"
        _quote = Symbol.string_to_symbol("quote")
        return Pair(_quote, Pair(scm_read(),NULL))
    elif syntax == "(":
        return read_tail()
    else:
